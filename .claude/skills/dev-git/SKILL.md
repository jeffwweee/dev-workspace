---
name: dev-git
description: "Git operations with conventional commits. Handles branching, committing, status checks, and repository management. Use for creating a new feature branch, committing changes, checking repository status, merging or rebasing branches. Triggers on 'git', 'commit', 'branch', 'merge', 'checkpoint'."
---
...
```bash
/dev-git --checkout-branch --create feature/login
```
...
```bash
/dev-git --commit --message "feat: Add user authentication"
```
...
```
User: /dev-git --commit --message "feat: Add user login"
