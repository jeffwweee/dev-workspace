---
name: dev-test
description: "Testing and verification. Runs tests, smoke tests, and verification to ensure code works correctly. Use for running test suite, verifying specific functionality, creating smoke tests, reproducing bugs, verifying fixes. Triggers on 'test', 'run tests', 'verify', 'smoke test'."
---
...
```bash
/dev-test --reproduce "Login fails with invalid credentials"
```
...
```bash
/dev-test --verify-fix "Login now handles invalid credentials"
```
