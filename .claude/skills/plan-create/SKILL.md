---
name: plan-create
description: Use when you have a spec or requirements for a multi-step task, before touching code. Creates bite-sized implementation plans from design documents or requirements. Triggers on "create plan", "implementation plan", "write plan", "plan this feature".
---
...
**Announce at start:** "I'm using the plan-create skill to create the implementation plan."
...
After comm-brainstorm skill creates a design doc:

```bash
/plan-create --design docs/plans/2026-02-24-feature-design.md
```
...
```bash
/plan-create "Add retry mechanism to API client"
```
...
**1. plan-parallel (this session)** - I dispatch fresh subagent per task, review between tasks, fast iteration

**2. Parallel Session (separate)** - Open new session with plan-execute, batch execution with checkpoints

**Which approach?"**

**If plan-parallel chosen:**
- **REQUIRED SUB-SKILL:** Use `plan-parallel` skill
- Stay in this session
- Fresh subagent per task + code review

**If Parallel Session chosen:**
- Guide them to open new session in worktree
- **REQUIRED SUB-SKILL:** New session uses `plan-execute` skill

## Integration with task-register
...
/plan-execute --plan docs/plans/2026-02-24-retry-implementation.md
# OR
/plan-parallel --plan docs/plans/2026-02-24-retry-implementation.md

# Step 5: Update task status when complete
/task-register --update-task TASK-015 --status completed --passes true
```
