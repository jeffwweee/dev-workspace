...
function loadConfig(): OrchestrationConfig {
  ...
  return yaml.load(content) as OrchestrationConfig;
}

/**
 * Get bot by role
 */
function getBotByRole(role: string, config: OrchestrationConfig): BotConfig | undefined {
 ...
 * Read task file
 */
function readTaskFile(taskId: string, role: string): { title: string } | null {
 ...
 * Read progress file
 */
function readProgressFile(taskId: string): any {
 ...
 */
async function publishToOutbox(botId: string, chatId: number, text: string, parseMode: string = 'MarkdownV2'): Promise<void> {
 ...
 */
async function sendNotification(type: string, options: any): Promise<void> {
  ...
  switch (type) {
    ...
    case 'complete': {
      if (options.details) {
        ...
        const filesList = files.map((f: string) => `  • ${escapeMarkdownV2(f)}`).join('\n');
