import { getStatePath, getRegistryPath, readJson, atomicWrite, auditLog, listSessions, getSession, deleteSession, isSessionOld, isLockExpired } from '../state/manager.js';
...
export async function cleanup(options) {
    ...
    const activeWorktreePaths = new Set(activeSessions
...
export async function pruneWorktrees(options) {
    ...
    const activeWorktreePaths = new Set(sessions
        .filter(s => s.worktreePath)
        .map(s => s.worktreePath));
    const removed = [];
    const failed = [];
