import { listSessions, getSession, deleteSession, updateLastActivity, isSessionOld } from '../state/manager.js';
/**
 * List all sessions
 */
export async function listSessionsCmd(showAll = false) {
    const sessions = listSessions();
    const filtered = showAll ? sessions : sessions.filter(s => s.status === 'active');
    const display = filtered.map(s => ({
        id: s.id,
