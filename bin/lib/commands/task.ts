import { getStatePath, getRegistryPath, atomicWrite, readJson, auditLog, type Session, type QueueItem } from '../state/manager.js';
import fs from 'fs';
...
export async function pickNext(options: {
  ...
  const queueData = readJson<{ queue: QueueItem[] }>(queuePath, { queue: [] });
  const projectsData = readJson<{ projects: any[] }>(projectsPath, { projects: [] });
  ...
  if (options.project) {
    const project = projectsData.projects.find(
      p => p.id === options.project || p.name === options.project
    );
...
export async function recordResult(options: {
  ...
  // Update task status
  task.status = options.status.toLowerCase() as any;
  atomicWrite(queuePath, queueData);
...
export async function showQueue(): Promise<Record<string, unknown>> {
  const queuePath = getStatePath('queue.json');
  const queueData = readJson<{ queue: QueueItem[] }>(queuePath, { queue: [] });
