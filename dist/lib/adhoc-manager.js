import { spawnAgent, killAgent, isAgentRunning } from './spawn-agent';
import { getLimits } from './orchestration-config';
const adhocAgents = new Map();
/**
 * Gets current adhoc counts by type
 */
export function getAdhocCounts() {
    const counts = {};
 ...
 */
export function canSpawnAdhoc(type) {
 ...
 */
export function spawnAdhocAgent(type, options = {}) {
 ...
 */
export function killAdhocAgent(sessionName) {
 ...
 */
export function updateAdhocActivity(sessionName) {
    const info = adhocAgents.get(sessionName);
    if (info) {
 ...
 */
export function findIdleAdhocAgents(timeoutMs) {
    const limits = getLimits();
 ...
 */
export function cleanupIdleAdhocAgents(timeoutMs) {
 ...
 */
export function listAdhocAgents() {
    const result = [];
    for (const [sessionName, info] of adhocAgents) {
        result.push({
            sessionName,
...
//# sourceMappingURL=adhoc-manager.js.map