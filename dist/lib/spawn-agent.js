...
import { isRoleSkill } from './role-loader.js';
export const CORE_AGENTS = ['backend', 'frontend', 'qa', 'review-git'];
/**
 ...
 */
function waitForSkillLoad(sessionName, skill, maxWaitMs = 30000) {
...
export function spawnAgent(options) {
    const { name, persona, role, skills = [], memoryFile, isAdhoc = false, model = 'sonnet', style = 'professional' } = options;
 ...
 */
export function killAgent(name, isAdhoc = false) {
    const sessionName = isAdhoc ? `cc-adhoc-${name}` : `cc-${name}`;
    try {
 ...
 */
export function isAgentRunning(name, isAdhoc = false) {
    const sessionName = isAdhoc ? `cc-adhoc-${name}` : `cc-${name}`;
    try {
        execSync(`tmux has-session -t ${sessionName} 2>/dev/null`);
        return true;
 ...
 */
export function sendToAgent(name, command, isAdhoc = false) {
    const sessionName = isAdhoc ? `cc-adhoc-${name}` : `cc-${name}`;
