 ...
 */
export async function notifyBlocked(task, agent, reason) {
    const bot = getBotByRole('orchestrator');
 ...
 */
export async function notifyFailed(task, agent, error) {
    const bot = getBotByRole('orchestrator');
    const adminChat = bot?.permissions?.admin_users?.[0];
    if (!adminChat) {
        return { success: false, error: 'No admin chat configured' };
 ...
 */
export async function notifyComplete(task, workflow, duration) {
    const bot = getBotByRole('orchestrator');
    const adminChat = bot?.permissions?.admin_users?.[0];
    if (!adminChat) {
        return { success: false, error: 'No admin chat configured' };
 ...
 */
export async function notifyAgentConflict(task, agentType, occupiedBy, queueLength) {
    const bot = getBotByRole('orchestrator');
    const adminChat = bot?.permissions?.admin_users?.[0];
    if (!adminChat) {
        return { success: false, error: 'No admin chat configured' };
 ...
 */
export async function notifyReviewRejected(task, confidence, threshold, issues) {
    const bot = getBotByRole('orchestrator');
    const adminChat = bot?.permissions?.admin_users?.[0];
    if (!adminChat) {
        return { success: false, error: 'No admin chat configured' };
